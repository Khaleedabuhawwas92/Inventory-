...
async function load(page = 1) {
  loading.value = true;
  try {
    const { data } = await stockService.listOut({ page, warehouse: filters.warehouse || undefined, status: filters.status || undefined });
    items.value = data.data;
    meta.value = data.meta;
  } catch (err) {
    toast.error('تعذر تحميل سندات الإخراج');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div>
    <div class="flex items-center justify-between flex-wrap gap-3 mb-6">
      <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">سندات إخراج البضاعة</h1>
      <router-link v-if="auth.can('stock.out')" to="/stock/out/new" class="btn-primary">+ سند إخراج جديد</router-link>
    </div>
    ...
    <div class="card overflow-x-auto">
      ...
      <EmptyState v-else-if="!items.length" title="لا توجد سندات إخراج" />
      <table v-else class="table-base">
        <thead><tr><th>رقم السند</th><th>التاريخ</th><th>المخزن</th><th>النوع</th><th>عدد الأصناف</th><th>الحالة</th><th></th></tr></thead>
        <tbody>
          <tr v-for="doc in items" :key="doc._id">
            ...
            <td>{{ OUTPUT_TYPE_LABELS[doc.outputType] }}</td>
            <td>{{ doc.items?.length }}</td>
            <td><StatusBadge :status="doc.status" /></td>
            <td><router-link :to="`/stock/out/${doc._id}`" class="btn-outline !px-2 !py-1 text-xs">عرض</router-link></td>
          </tr>
        </tbody>
      </table>
