...
async function load(page = 1) {
  loading.value = true;
  try {
    const { data } = await stockService.listAdjustments({ page, warehouse: filters.warehouse || undefined, status: filters.status || undefined });
    items.value = data.data;
    meta.value = data.meta;
  } catch (err) {
    toast.error('تعذر تحميل سندات التسوية');
  } finally {
    loading.value = false;
  }
}
...
<template>
  <div>
    <div class="flex items-center justify-between flex-wrap gap-3 mb-6">
      <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">تسويات المخزون</h1>
      <router-link v-if="auth.can('stock.adjust')" to="/stock/adjustments/new" class="btn-primary">+ تسوية جديدة</router-link>
    </div>
    ...
    <div class="card overflow-x-auto">
      ...
      <EmptyState v-else-if="!items.length" title="لا توجد تسويات" />
      <table v-else class="table-base">
        <thead><tr><th>رقم السند</th><th>التاريخ</th><th>المخزن</th><th>عدد الأصناف</th><th>الحالة</th><th></th></tr></thead>
        <tbody>
          <tr v-for="doc in items" :key="doc._id">
            ...
            <td><router-link :to="`/stock/adjustments/${doc._id}`" class="btn-outline !px-2 !py-1 text-xs">عرض</router-link></td>
          </tr>
        </tbody>
      </table>
