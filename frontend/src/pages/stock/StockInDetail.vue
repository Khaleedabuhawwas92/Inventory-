...
async function load() {
  loading.value = true;
  try {
    const { data } = await stockService.getIn(route.params.id);
    doc.value = data.data;
  } catch (err) {
...
async function approveDoc() {
  ...
  busy.value = true;
  try {
    await stockService.approveIn(doc.value._id);
...
async function cancelDoc() {
  ...
  busy.value = true;
  try {
    await stockService.cancelIn(doc.value._id);
    ...
    <template v-else-if="doc">
      <div class="flex items-center justify-between flex-wrap gap-3 mb-6 print:hidden">
        <div>
          ...
          <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100 mt-1">سند إدخال {{ doc.docNo }}</h1>
        </div>
        <div class="flex items-center gap-2">
          <button class="btn-outline" @click="safePrint()">طباعة</button>
          <button v-if="doc.status === 'DRAFT' && auth.can('stock.in')" class="btn-primary" :disabled="busy" @click="approveDoc">اعتماد</button>
          <button v-if="doc.status !== 'CANCELLED' && auth.can('stock.in')" class="btn-danger" :disabled="busy" @click="cancelDoc">إلغاء</button>
        </div>
      </div>
      ...
      <div class="card p-6">
        ...
        <table class="table-base">
          <thead><tr><th>الصنف</th><th>SKU</th><th>الكمية</th><th>تكلفة الوحدة</th><th>الإجمالي</th></tr></thead>
          <tbody>
